# Governor Limits and Performance Optimization (Research Layer)
...
This topic covers Salesforce governor limits, performance optimization strategies, SOQL query optimization, selective query patterns, and resource management best practices. This is an **anchor topic** that connects directly to existing RAG content in `rag/development/apex-patterns.md`, `rag/patterns/cross-cutting-patterns.md`, and `rag/integrations/sis-sync-patterns.md`, which mention governor limits and optimization but don't provide comprehensive guidance on limit types, optimization techniques, and performance tuning strategies.

This research provides deeper ecosystem guidance on governor limits (SOQL, DML, CPU time, heap size, callouts), query selectivity, performance optimization patterns, and resource monitoring that are widely accepted in the Salesforce community. These patterns are especially relevant for integration-heavy systems with large data volumes, as seen in `rag/integrations/...`, where proper limit management becomes critical for system performance and scalability.

This relates to the **development** and **performance** domains in `rag/**`, specifically Apex optimization, SOQL design, and integration performance patterns.
...
- The RAG file mentions governor limits and optimization (SOQL optimization, bulkification, monitoring limits) but focuses on implementation patterns. This research adds comprehensive guidance on specific governor limit types, limit values, query selectivity rules, and advanced optimization techniques that are widely accepted in the Salesforce community.

**Connection to `rag/patterns/cross-cutting-patterns.md`**:
- The RAG file mentions governor limit management as a cross-cutting pattern and links to domain-specific documentation. This research adds detailed guidance on limit monitoring, proactive checking, and optimization strategies that complement the pattern summary.

**Connection to `rag/integrations/sis-sync-patterns.md`**:
- The RAG file describes high-volume batch synchronization patterns but focuses on integration-specific patterns. This research adds broader governor limit guidance that applies to all high-volume scenarios, not just SIS syncs.

**Gaps filled by this research**:
- The RAG files mention governor limits but don't provide comprehensive guidance on specific limit types and values, query selectivity rules, heap size optimization, or limit monitoring patterns. This research adds detailed guidance on these topics that are essential for performance optimization.

**Nuances added**:
- This research emphasizes query selectivity rules (10% threshold, indexed fields), specific governor limit values, heap size optimization patterns, and proactive limit monitoring that complement the RAG files' focus on bulkification and asynchronous patterns.
...
These tradeoffs are **not** automatic candidates for RAG changes; they require human judgment based on specific use cases, data volumes, and performance requirements.

## 6. Candidate Ideas for RAG Enhancement

- **Expand governor limits guidance in `rag/development/apex-patterns.md`**: The RAG file mentions governor limits but could include specific limit values, limit monitoring patterns, and proactive limit checking strategies.

- **Add query selectivity guidance**: Consider adding detailed guidance on query selectivity rules (10% threshold, indexed fields, selective query patterns) to complement existing SOQL optimization guidance.

- **Add heap size optimization patterns**: Consider adding guidance on heap size optimization, transient variables, and memory management patterns that complement existing bulkification guidance.

- **Expand asynchronous patterns**: The RAG file mentions asynchronous patterns but could include more detailed guidance on when to use Batch vs Queueable vs Scheduled, batch size optimization, and job monitoring patterns.

- **Add limit monitoring and logging patterns**: Consider adding guidance on using the Limits class, proactive limit checking, and limit usage logging for optimization.
...
(Detailed sources with URLs are maintained in `Knowledge/research/sources/governor-limits-sources.md`)
